...
for kmax in (100.0, 300.0, 1000.0):
    # k_max goes through the CONSTRUCTOR. Assigning turbulence.K_MAX here --
    # which is what this script used to do -- changes nothing, because the
    # default is bound into vertical_mixing's signature at import time. That
    # is what made P-40's ladder return three identical survival counts and
    # get recorded as an elimination.
    m = build_on(SigmaLevels(20), 4000.0, 8, k_max=kmax)
    dt = m.max_dt()
    done = 0
    for _ in range(12):
        m.run(3600, dt=dt)
